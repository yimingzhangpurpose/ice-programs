...
def program6(fileloc1, fileloc2, filetype1, filetype2, ticker):
    ...
    dirpath = "phase1_output/program6Q_out/plotly/" + filetype1 + "/" + filetype2 + "/" + date + "/"
...
def yeda6(fileloc1, fileloc2, filetype1, filetype2, ticker):
    ...
    date = fsplit[len(fsplit) - 1][:-4]
    os.chdir(str(pathlib.Path.home()) + "/workspace/")
    dirpath = "phase1_output/program6Q_out/matplotlib/" + filetype1 + "/" + filetype2 + "/" + date + "/"
    pathlib.Path(dirpath).mkdir(parents=True, exist_ok=True)

    print("Getting data...")
    df1 = pf.get_tickerData2(fileloc1, filetype1, "X:SXBTUSD", "Q")
    print("X:SXBTUSD", ticker)
    df2 = pf.get_tickerData2(fileloc2, filetype2, ticker, "Q")
    print("Got data. Generating plot...")
