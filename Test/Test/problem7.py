...
def program7(filetype1, filetype2, ticker):
    ...
    dirpath = "phase1_output/program7_out/plotly/" + filetype1 + "/" + filetype2 + "/"
...
def yeda7(fileloc1, fileloc2, filetype1, filetype2, ticker):
    init_tm = time()
    ...
    #date = fsplit[len(fsplit) - 1][:-4]
    os.chdir(str(pathlib.Path.home()) + "/workspace/")
    dirpath = "phase1_output/program7_out/matplotlib/" + filetype1 + "/" + filetype2 + "/"
    pathlib.Path(dirpath).mkdir(parents=True, exist_ok=True)

    print("Getting data...")
    df1 = get_closingPrice(fileloc1, "X:SXBTUSD")
    df2 = get_closingPrice(fileloc2, ticker)
    print("Got data. Generating plot...")
...
if __name__ == "__main__":
    try:
        ...
        print("Program 7: Plot Daily Spread (aggregated)")
