...
def get_fileList(filetype):
    ...
    """
    if (filetype == "A"):
        kk = sorted(glob("/space/data/new/PLUSTICK_1619_*"))
    if (filetype == "B"):
        kk = sorted(glob("/space/data/new/PLUSTICK_FI_1356_*"))
    if (filetype == "C"):
        kk = sorted(glob("/space/data/new/PLUSTICK_FUTURES_666_*"))
    if (filetype == "D"):
        kk = sorted(glob("/space/data/new/PLUSTICK_FUTURES_680_*"))
    projLists.file_list=[]
    for ii in kk:
        projLists.file_list.append(ii[-12:-4])
